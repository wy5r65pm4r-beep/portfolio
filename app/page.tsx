...
export default function Home() {
  ...
  return <main>
    ...
    <section className="works" id="works"><div className="works-head"><SectionTitle index="01" title="Selected Works" subtitle="一组经过筛选的商业设计案例，呈现从信息梳理到最终落地的完整能力。" /><div className="view-toggle" aria-label="项目视图切换"><button className={view === 'grid' ? 'active' : ''} onClick={() => setView('grid')}><Grid2X2 size={14} /> Grid</button><button className={view === 'index' ? 'active' : ''} onClick={() => setView('index')}><List size={15} /> Index</button></div></div><div className={`project-browser ${view}`}>{projects.map((project) => <a href={`${siteBase}/projects/${project.slug}/`} className={`project-card ${project.tone}`} key={project.no}><div className="project-media"><span className="asset-label">需要素材<br /><b>{project.asset}</b></span><span className="project-no">{project.no}</span></div><div className="project-info"><div><h3>{project.title}</h3><p>{project.en}</p></div><div><span>{project.type}</span><span>{project.year}</span></div><ArrowUpRight className="project-arrow" size={20} /></div></a>)}</div></section>
    <section className="case flagship" id="project-01"><SectionTitle index="02" title="SUPOR / 苏泊尔" subtitle="旗舰案例预览 · 用统一的视觉系统，让多主题、多信息的节日传播保持清晰与品牌一致。" /><div className="case-meta"><p><b>Challenge</b>在密集的促销信息与节日氛围之间建立秩序，同时维持系列海报的整体识别。</p><p><b>Role</b>视觉概念 / 版式系统 / 海报执行 / 延展落地</p><a href={`${siteBase}/projects/supor/`}>View project <ArrowUpRight size={15} /></a></div><div className="poster-grid"><MediaSlot label="苏泊尔主海报图" note="竖版主视觉 / 核心概念" ratio="portrait" /><MediaSlot label="系列海报图" note="节日主题海报 01—04" ratio="portrait" /><MediaSlot label="系列海报图" note="节日主题海报 05—08" ratio="portrait" /></div><MediaSlot label="苏泊尔应用场景图" note="建议：线下物料、商场灯箱或社交媒体应用合成" ratio="wide" /></section>
    <section className="case split" id="project-02"><div><SectionTitle index="03" title="MANLONG / 曼龙" /><h3>让插画成为儿童产品的想象入口，而不只是表面装饰。</h3><p>从帐篷形态、观看距离与印刷工艺出发，建立既有童趣又保持克制的插画叙事。</p><a className="text-link" href={`${siteBase}/projects/manlong/`}>Explore case <ArrowUpRight size={15} /></a></div><div className="stack"><MediaSlot label="帐篷产品图" note="完整产品 / 使用场景" ratio="square" /><MediaSlot label="插画细节图" note="图案局部 / 线稿过程" ratio="landscape" /></div></section>
    ...
    <section className="project-index" id="index"><div className="index-head"><SectionTitle index="09" title="Project Index" /><span>Selected archive / 2022—2026</span></div>{projects.map((p) => <a href={`${siteBase}/projects/${p.slug}/`} key={p.no}><span>{p.no}</span><h3>{p.title}</h3><p>{p.type}</p><time>{p.year}</time><ArrowUpRight size={18} /></a>)}</section>
